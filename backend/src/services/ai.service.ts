...
Внимательно анализируй текст. Если клиент в самом сообщении упоминает свою локацию (особенно за рубежом или из нетипичного города), эта текстовая информация ДОЛЖНА иметь высший приоритет над системным адресом. Поля адреса часто заполняются ошибочно, поэтому возвращай в \`normalizedLocation\` ту локацию, которую заявляет клиент в тексте.
...
3. ПРИОРИТЕТНОСТЬ (Шкала 1-10):
   ...
   - 4: Стандартные жалобы масса.
   - 3: Вопросы по конкретным продуктам, статус рассмотрения заявок (когда нет конфликта, но нужен ответ).
   - 2: Общие вопросы, пожелания или отзывы без негатива.
   - 1: Откровенный спам или пустые тикеты.
Важное правило: Если клиент сообщает о значительной проблеме БЕЗ негатива, все равно дай соответствующий приоритет проблеме. Если обычный клиент пишет о незначительной проблеме С НЕГАТИВОМ, не завышай приоритет слишком сильно (ниже чем у VIP).
...
6. ГЕО-НОРМАЛИЗАЦИЯ: Извлеки Город, Область, Страну для маршрутизации. Ориентируйся в первую очередь на данные из текста.
...
export class AIService {
    ...
    async analyzeTicket(
        ...
        try {
            ...
            let finalPriority = Number(parsed.priority) || 0;
            if (segment === 'VIP' && finalPriority < 8) {
                finalPriority = 8;
            }
